...
export function OnwardJobsPage({notify}:{notify:Notify}){
 ...
 return <div className="onward-jobs-dashboard">
  ...
  <section className="oj-hero">
   <div className="oj-hero-copy"><span className="eyebrow">RAW → DEDUPE → INDEX → VPS → ONWARD V1</span><h2>法国招聘数据管线</h2><p>21 天采集仓 + 14 天市场视图。Dashboard 只读派生状态，不读取原始岗位正文，也不会触发 API。</p></div>
  ...
  <section className="oj-kpis">
   ...
   <Kpi label="UNIQUE · 14D" value={compact(data.totals.unique14d)} detail={`${data.totals.duplicatePct.toFixed(1)}% raw→canonical dedupe`}/>
   <Kpi label="STAGE · 14D" value={full(data.totals.stage14d)} detail={`Alternance ${full(data.totals.alternance14d)}`} kind="green"/>
   <Kpi label="PARIS · 14D" value={compact(data.totals.paris14d)} detail={`Île-de-France ${full(data.totals.ileDeFrance14d)}`}/>
  ...
  <section className="panel oj-section"><div className="section-heading"><div><span className="eyebrow">SOURCE HEALTH</span><h2>API / 公共招聘源</h2><p>等待表示已完成当前轮、尚未到下次刷新；错误数来自近期日志尾部，不等于永久失效。</p></div><span className="small-tag">14 天 canonical contribution</span></div><SourceGrid sources={data.sources}/></section>
  ...
  <section className="panel oj-section"><div className="section-heading"><div><span className="eyebrow">FINANCE COVERAGE</span><h2>金融机构官方招聘源</h2><p>重点覆盖银行、CIB、资管、保险、PE / Private Debt；数字为当前官方源已保留记录，不与14天统计口径混淆。</p></div><span className="proof-badge"><Building2 size={13}/>{data.financeSources.length} sources</span></div><FinanceGrid rows={data.financeSources}/></section>
