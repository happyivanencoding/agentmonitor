...
chrome.storage.local.get('status').then(({status:s})=>{document.getElementById('observer').textContent=s?.observer?`Observer: ${s.observer} · 待发送 ${s.pendingEvents||0} · ${s.captureStatus||'等待捕捉'}${s.captureError?' · '+s.captureError:''}`:'等待 ChatGPT 响应';});
async function check(){try{await send({type:'heartbeat'});show('已连接本机 Agent Monitor · 配对有效。')}catch(e){show(e.message,true)}}
document.getElementById('check').onclick=check;
document.getElementById('pair-form').onsubmit=async e=>{e.preventDefault();try{await send({type:'pair',key:document.getElementById('key').value.trim()});document.getElementById('key').value='';show('已配对。观察器已注入打开的 ChatGPT 标签页。')}catch(e){show(e.message,true)}};
