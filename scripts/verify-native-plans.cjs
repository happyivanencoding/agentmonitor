...
(async()=>{const taskId=process.argv[2];assert(/^tsk_[a-z0-9]+$/.test(taskId||''),'Provide a real local task ID');
 const snapshot=await (await fetch('http://127.0.0.1:43218/api/snapshot')).json();assert.equal(snapshot.version,'0.5.4');assert(!snapshot.stale);const task=snapshot.tasks.find(t=>t.id===taskId);assert(task,'Native task must exist in the real snapshot');
 ...
 await item.scrollIntoViewIfNeeded();await page.screenshot({path:path.resolve('.local/native054-desktop.png')});
 await page.setViewportSize({width:420,height:860});await item.scrollIntoViewIfNeeded();await page.screenshot({path:path.resolve('.local/native054-mobile.png')});assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+2),'No global mobile horizontal overflow');assert.deepEqual(errors,[]);
 const report={source:'installed Monitor / real native Task',version:snapshot.version,taskId,steps:task.steps.length,completed:task.steps.filter(s=>s.status==='completed').length,currentStep:task.steps.find(s=>s.status==='in_progress')?.id,updatedAt:task.updatedAt,nativePlanExpandableWithoutAcp:true,desktopAndMobile:true,pageErrors:errors};await fs.writeFile(path.resolve('.local/native054-'+Date.now()+'.json'),JSON.stringify(report,null,2));console.log(JSON.stringify(report,null,2));
