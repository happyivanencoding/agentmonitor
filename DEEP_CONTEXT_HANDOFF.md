...
## Current release: 0.5.4 — verified capture and independent native progress

The current per-user release is **0.5.4**. Do not repeat the previous diagnosis that the owner merely forgot to reload: the failed deployment really had a running 0.5.2 extension, two Request rows and zero tool rows. `observer-ready` only proved liveness. Real acceptance recovered seven tool records for an existing Request and connected its exact checkpoint Task to **3/3 real steps**. A different conversation's attachment-only user message was also captured. Full acceptance and remaining boundaries: [VERIFICATION_0_5_4.md](docs/VERIFICATION_0_5_4.md).
